...
Serial.begin(115200);
   ...
   for (int i=0; i<800;i++)
   {
    sum+=cs_4_8.capacitiveSensor(20);
    Serial.println(sum);
    i++;
   }
   capstart=sum/800;
   Serial.println(capstart);
   delay(3000);
Setupaccelerometer();
...
void fadeInAndOut(uint8_t red, uint8_t green, uint8_t blue) {
int b;
int i; 
int ledtime;
...
void calculatedrinktime()
{
      ...
      while(checkdrinking())
      {
          while((pixel<20) && (checkdrinking()))
          {
            ...
            {
              strip.setPixelColor(pixel,100,120,255);
              strip.show();
              /*
